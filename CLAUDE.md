...
NetHack Next is a NetHack-inspired roguelike written in C for the **ZX Spectrum
...
```bat
build.bat            REM builds the whole game -> nhnext.nex (+ nhnext.map)
build.bat foo.c      REM builds a single .c file -> foo.nex
run.bat              REM runs nhnext.nex in CSpect
...
```
zcc +zxn -subtype=nex -vn -SO3 -clib=sdcc_iy --max-allocs-per-node200000 -m <srcs> -o nhnext -create-app
```
...
| `game.h` | shared player/run state (`extern`s defined in `nhnext.c`) |
| `nhnext.c` | game state definitions, main loop, map/status rendering, title screen |

Shared mutable state (`hero_x/y`, `dlvl`, `php`, `gold`, `ac`, `xp`, `nutrition`, …)
is declared `extern` in `game.h` and **defined once in `nhnext.c`**. Modules include
...
- **Rendering is a single full redraw per turn** (`draw_map` in `nhnext.c`) using a
...
- `build_level()` (in `nhnext.c`) orchestrates: `gen_level()` → spawn monsters →
...
`__BSS_END_tail` vs `__register_sp` in `nhnext.map` after adding any. This already bit
